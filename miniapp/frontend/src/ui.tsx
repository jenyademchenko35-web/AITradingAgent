...
export const StatusBadge = memo(function StatusBadge({ status }: { status: string | null | undefined }) {
  ...
  return <span className={`status-badge status-${toneFor(text)}`}>{text}</span>;
});
...
export function EmptyState({ title, detail }: { title: string; detail: string }) {
  return <div className="empty-state"><b>{title}</b><p>{detail}</p></div>;
}
...
export function SearchBar({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return <input className="search-bar" aria-label="Поиск символа" placeholder="BTC, BNB, BONK…" value={value} onChange={(event) => onChange(event.target.value)} />;
}
...
export function FilterBar({ filter, sort, onFilter, onSort }: {
  ...
  return <div className="filter-bar"><div role="group" aria-label="Фильтр сигналов">{(["ALL", "SETUP", "WATCH", "LIVE", "BLOCKED"] as SignalFilter[]).map((item) => <button className={filter === item ? "active" : ""} key={item} onClick={() => onFilter(item)}>{item === "ALL" ? "Все" : item}</button>)}</div><label>Sort<select aria-label="Сортировка сигналов" value={sort} onChange={(event) => onSort(event.target.value as SignalSort)}><option value="confidence">Confidence</option><option value="symbol">Symbol</option><option value="status">Status</option><option value="updated">Updated</option></select></label></div>;
}
