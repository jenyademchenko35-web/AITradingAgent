...
function DashboardHome({ dashboard, stats, watchlist, system, activity, shadow, loading, onNavigate, onSignal }: {
  ...
  const closedShadow = shadow?.closed ?? null;
  return <><section className="dashboard-hero pro"><div><small>TRADEWATCHER · READ ONLY</small><h2>{publishedStatus(system?.server ?? dashboard?.status)}</h2><p>Snapshot {freshnessLabel(system?.freshness?.agent_snapshot)} · {formatTime(system?.last_cycle_timestamp ?? dashboard?.updated_at)}</p></div><div className="hero-facts"><div><span>Read Only</span><b>{system?.read_only === true ? "ACTIVE" : "—"}</b></div><div><span>Auto Refresh</span><b>30 sec</b></div></div></section>
    <section className="panel dashboard-section"><div className="section-heading"><div><h2>System</h2><p>Статусы и их свежесть из Runtime API.</p></div></div><div className="health-grid"><HealthCard label="Server" status={publishedStatus(system?.server)} detail={freshnessLabel(system?.freshness?.agent_snapshot)} /><HealthCard label="Agent" status={publishedStatus(system?.agent)} detail={freshnessLabel(system?.freshness?.agent_snapshot)} /><HealthCard label="Telegram" status={publishedStatus(system?.telegram)} detail="Published status" /><HealthCard label="Research" status={publishedStatus(system?.research_integrity_state ?? system?.research)} detail="Integrity, not server health" /><HealthCard label="News" status={publishedStatus(system?.news)} detail={freshnessLabel(system?.freshness?.news)} /></div><div className="dashboard-grid"><MetricCard label="Cycle" value={system?.cycle} detail={system ? undefined : "Not published"} /><MetricCard label="Next Scan (sec)" value={system?.next_cycle_seconds} detail={system?.next_cycle_seconds == null ? "Not published" : undefined} /><MetricCard label="Uptime (sec)" value={system?.uptime_seconds} detail={system?.uptime_seconds == null ? "Not published" : undefined} /><MetricCard label="Read Only" value={system?.read_only === true ? "ACTIVE" : null} detail={system?.read_only === true ? undefined : "Not published"} /></div></section>
    <section className="panel dashboard-section"><div className="section-heading"><div><h2>Metrics</h2><p>Source: {dashboard?.metrics_source || "UNKNOWN"}</p></div><StatusBadge status={dashboard?.metrics_available ? "PUBLISHED" : "NOT PUBLISHED"} /></div><div className="dashboard-grid"><MetricCard label="Winrate" value={dashboard?.winrate == null ? null : `${dashboard.winrate}%`} detail={dashboard?.metrics_available ? undefined : "Not published"} /><MetricCard label="Profit Factor" value={dashboard?.profit_factor} detail={dashboard?.metrics_available ? undefined : "Not published"} /><MetricCard label="Net R" value={stats.net_r} detail={stats.net_r === undefined ? "Not published" : "Source: trades.csv"} /><MetricCard label="Closed Trades" value={stats.closed_trades} detail={stats.closed_trades === undefined ? "Not published" : "Source: trades.csv"} /><MetricCard label="Open Trades" value={dashboard?.open_trades} detail={dashboard?.metrics_available ? undefined : "Not published"} /><MetricCard label="Shadow Active" value={activeShadow?.length ?? null} detail={activeShadow === null ? "Not published" : "Research Lab shadow book"} /></div></section>
    {shadow && <section className="panel dashboard-section"><div className="section-heading"><div><h2>Shadow Monitoring</h2><p>Отдельный Research Lab shadow book.</p></div><button className="text-action" onClick={() => onNavigate("shadow")}>Open →</button></div><div className="dashboard-grid"><MetricCard label="Active" value={activeShadow?.length ?? null} detail={activeShadow === null ? "Not published" : undefined} /><MetricCard label="Closed" value={closedShadow?.length ?? null} detail={closedShadow === null ? "Not published" : undefined} /><MetricCard label="Updated" value={formatTime(shadow.updated)} /><MetricCard label="Symbols" value={shadow.symbols?.length ?? null} detail={shadow.symbols === null ? "Not published" : undefined} /></div></section>}
    <section className="panel dashboard-section"><div className="section-heading"><div><h2>Quick Signals</h2><p>Существующий watchlist без дополнительных запросов.</p></div><StatusBadge status={`${watchlist.length} published`} /></div>{watchlist.length ? <div className="quick-signal-list">{watchlist.slice(0, 6).map((item) => <button className="quick-signal" key={`${item.symbol}-${item.timeframe}`} onClick={() => onSignal(item)}><b>{item.symbol}</b><StatusBadge status={freshnessLabel(item.freshness) === "FRESH" ? item.status : "STALE"} /><span>{item.side || "—"}</span><strong>{item.confidence}%</strong><small>{freshnessLabel(item.freshness)}</small></button>)}</div> : <EmptyState title="No data published" detail="The watchlist did not publish signals. Mini App displays data only." />}</section>
    <section className="panel dashboard-section activity"><div className="section-heading"><div><h2>Live Activity</h2><p>Последние timestamped события из Runtime API.</p></div></div>{hasActivity ? <div className="activity-feed">{activity.map((event, index) => <div className="activity-row" key={`${event.timestamp}-${event.type}-${index}`}><div><span>{activityLabel(event)}</span><small>{freshnessLabel(event.freshness)}{event.symbol ? ` · ${event.symbol}${event.timeframe ? ` · ${event.timeframe}` : ""}${event.status ? ` · ${event.status}` : ""}` : ""}</small></div><b>{formatTime(event.timestamp)}</b></div>)}</div> : <EmptyState title="No activity published" detail="Runtime API did not publish timestamped events." />}</section>
...
function ResearchSummary({ report }: { report: ResearchLiveReport }) {
  ...
  return <div className="research-summary"><h3>Runtime status</h3><div className="research-grid"><MetricCard label="Status name" value={status.name} /><MetricCard label="State" value={status.state} /><MetricCard label="Enabled" value={status.enabled} /><MetricCard label="Recommendation" value={status.recommendation} /></div><h3>Research integrity</h3>{Object.keys(integrity).length ? <div className="research-grid"><MetricCard label="Integrity state" value={metricValue(integrity.integrity_state ?? integrity.state)} /><MetricCard label="Ledger closed" value={metricValue(integrity.ledger_closed)} /><MetricCard label="Canonical outcomes" value={metricValue(integrity.canonical_outcomes)} /><MetricCard label="Sync gap" value={metricValue(integrity.sync_gap)} /><MetricCard label="Feature join coverage" value={metricValue(integrity.feature_join_coverage)} /><MetricCard label="Historical unresolved" value={metricValue(integrity.historical_unresolved_joins)} /><MetricCard label="Current unresolved" value={metricValue(integrity.current_pipeline_unresolved_joins)} /><MetricCard label="New fully joined" value={metricValue(integrity.new_outcomes_fully_joined)} /><MetricCard label="Ranking" value={integrity.ranking_allowed === true ? "ALLOWED" : "BLOCKED"} /><MetricCard label="Walk Forward" value={integrity.walk_forward_allowed === true ? "ALLOWED" : "BLOCKED"} /><MetricCard label="Promotion" value={integrity.promotion_allowed === true ? "ALLOWED" : "BLOCKED"} /></div> : <EmptyState title="Integrity not published" detail="The production runtime bundle did not include Research Data Integrity." />}<h3>Best candidate</h3><div className="research-grid"><MetricCard label="Strategy" value={candidate.strategyName} /><MetricCard label="PF" value={candidate.profitFactor} /><MetricCard label="Winrate" value={candidate.winrate} /><MetricCard label="Closed trades" value={candidate.closedTrades} /><MetricCard label="Net R" value={candidate.netR} /><MetricCard label="Status" value={candidate.status} /></div><MetricCard label="Promotion progress" value={researchText(report.promotion_probability)} /><MetricCard label="Recommendation" value={researchText(report.recommendation)} />{report.ranking.length ? <section className="research-runtime-list"><h3>Ranking</h3><ul>{report.ranking.map((item, index) => <li key={`${researchItemLabel(item)}-${index}`}>{researchItemLabel(item)}</li>)}</ul></section> : <EmptyState title="Ranking unavailable" detail="Runtime API did not publish a research ranking." />}{(report.top_features.length || report.worst_features.length) ? <section className="feature-runtime-list"><div><h3>Top Features</h3><ul>{report.top_features.map((item, index) => <li key={`${researchItemLabel(item)}-${index}`}>{researchItemLabel(item)}</li>)}</ul></div><div><h3>Worst Features</h3><ul>{report.worst_features.map((item, index) => <li key={`${researchItemLabel(item)}-${index}`}>{researchItemLabel(item)}</li>)}</ul></div></section> : <EmptyState title="Feature importance unavailable" detail="Feature analysis requires fully joined canonical outcomes; unavailable fields are not inferred." />}</div>;
}
