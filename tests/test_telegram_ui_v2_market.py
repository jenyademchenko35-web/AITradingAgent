...
def test_market_uses_latest_status_per_symbol():
    ...
    assert "BTC  🟢 ГОТОВ К ВХОДУ" in text
    assert "ETH  ⚪ НЕТ СДЕЛКИ" in text
...
def test_market_keyboard_routes_symbols_and_refresh():
    ...
    assert callbacks == ["ui:v2:symbol:BTCUSDT", "ui:v2:market", "ui:v2:home"]
...
def test_compact_trade_and_stats_wrappers():
    ...
    assert "Открыто: 1" in trades
    assert "Closed: 10" in stats and "PF: 1.50" in stats
