...
def test_export_aggregates_chunks_and_produces_canonical_hash(tmp_path: Path) -> None:
    ...
    client = _client_for(rows=rows)
    report = export_symbol(client=client, symbol="EUR/USD", start="2023-01-01", end="2023-02-01", output_dir=tmp_path)
    ...
    repeated = export_symbol(client=_client_for(rows=rows), symbol="EUR/USD", start="2023-01-01", end="2023-02-01", output_dir=tmp_path / "repeat")
...
def test_duplicate_invalid_or_nonfinite_data_fail_without_writing(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="duplicate"):
        export_symbol(client=_client_for(rows=duplicate), symbol="EUR/USD", start="2023-01-01", end="2023-02-01", output_dir=tmp_path)
    ...
    with pytest.raises(DukascopyExportError):
        export_symbol(client=_client_for(rows=malformed), symbol="EUR/USD", start="2023-01-01", end="2023-02-01", output_dir=tmp_path)
    nonfinite = [_raw("2023-01-02T00:00:00+00:00")]; nonfinite[0]["close"] = "NaN"
    with pytest.raises(DukascopyExportError):
        export_symbol(client=_client_for(rows=nonfinite), symbol="EUR/USD", start="2023-01-01", end="2023-02-01", output_dir=tmp_path)
...
def test_http_client_retries_then_succeeds_and_permanent_failure_is_bounded() -> None:
    ...
    def flaky(*_: object, **__: object) -> _Response:
        attempts.append(None)
        if len(attempts) == 1:
            raise OSError("temporary")
        return _Response({"candles": [_raw("2023-01-02T00:00:00+00:00")]})
    ...
    assert len(client.fetch_chunk(symbol="EUR/USD", chunk=chunk, price_side="BID")) == 1 and len(attempts) == 2
...
def test_no_overwrite_without_force_and_runtime_crypto_state_remains_untouched(tmp_path: Path) -> None:
    ...
    export_symbol(client=_client_for(rows=rows), symbol="EUR/USD", start="2023-01-01", end="2023-02-01", output_dir=tmp_path)
...
def test_run_export_reports_next_offline_command_and_data_path_is_ignored(tmp_path: Path) -> None:
    ...
    client = _Client({("EUR/USD", "2023-01-01T00:00:00+00:00"): eur, ("GBP/USD", "2023-01-01T00:00:00+00:00"): gbp})
    report = run_export(symbols=("EUR/USD", "GBP/USD"), start="2023-01-01", end="2023-01-03", output_dir=tmp_path, client=client)
